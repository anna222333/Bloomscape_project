...
## QG-DOC2: Evidence Stored in Repo, Size Controlled
- Evidence (screenshots/log snippets) may be stored in the repo, but must remain lightweight.
Acceptance:
- Evidence paths are listed in stage reports (`docs/REPORTS/*`).
- Large binaries are avoided; if unavoidable, document size impact and rationale.
