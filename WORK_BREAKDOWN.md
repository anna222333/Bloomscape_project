...
## Cross-stage output rule
For any stage execution, the Stage Chat must produce:
- Updated Stage Brief (status + closure record with AT results)
- Stage Report under `docs/REPORTS/` (summary + evidence paths)
- Any supporting capture docs (e.g., `docs/DISCOVERY/*` for Stage B)
- Risk updates in `RISK_REGISTER.md` when new risks appear
